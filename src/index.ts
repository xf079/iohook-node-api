...
// @ts-ignore
const lib = require('node-gyp-build')(join(__dirname, '..'))
...
interface IOHookEvents {
  'input': (e: UiohookKeyboardEvent | UiohookMouseEvent | UiohookWheelEvent) => void;
  'keydown': (e: UiohookKeyboardEvent) => void;
  'keyup': (e: UiohookKeyboardEvent) => void;
  'mousedown': (e: UiohookMouseEvent) => void;
  'mouseup': (e: UiohookMouseEvent) => void;
  'mousemove': (e: UiohookMouseEvent) => void;
  'click': (e: UiohookMouseEvent) => void;
  'wheel': (e: UiohookWheelEvent) => void;
}
...
class IOHook extends EventEmitter {
  ...
  private handler(e: UiohookKeyboardEvent | UiohookMouseEvent | UiohookWheelEvent) {
    ...
    switch (e.type) {
      case EventType.EVENT_KEY_PRESSED:
        this.emit('keydown', e as UiohookKeyboardEvent)
        break
      case EventType.EVENT_KEY_RELEASED:
        this.emit('keyup', e as UiohookKeyboardEvent)
        break
      case EventType.EVENT_MOUSE_CLICKED:
        this.emit('click', e as UiohookMouseEvent)
        break
      case EventType.EVENT_MOUSE_MOVED:
        this.emit('mousemove', e as UiohookMouseEvent)
        break
      case EventType.EVENT_MOUSE_PRESSED:
        this.emit('mousedown', e as UiohookMouseEvent)
        break
      case EventType.EVENT_MOUSE_RELEASED:
        this.emit('mouseup', e as UiohookMouseEvent)
        break
      case EventType.EVENT_MOUSE_WHEEL:
        this.emit('wheel', e as UiohookWheelEvent)
        break
    }
  }
  ...
  keyTap(key: number, modifiers: number[] = []) {
    if (modifiers.length > 0) {
      ...
      for (const modifier of modifiers) {
        lib.keyTap(modifier, KeyToggle.Down)
      }
  ...
  // 添加类型声明以支持事件监听
  on<E extends keyof IOHookEvents>(event: E, listener: IOHookEvents[E]): this {
    return super.on(event, listener);
  }

  emit<E extends keyof IOHookEvents>(event: E, ...args: Parameters<IOHookEvents[E]>): boolean {
